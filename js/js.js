...
if(ddd>0){
	...
	var canlen = ii.getElementsByClassName('canvas').length;
		...
		var g, qa;
 		for(i=0;i<canlen;i++){
			...
			g=g.replace(/\D+/g,"");
			if(qa[0]=='.'){
				canva[i].innerHTML='<div style="position:absolute;z-index:5;display:none;">'+g+'</div>';
			}else{
				if(document.getElementById('width'+g).value<1000 || document.getElementById('height'+g).value<400){
					canva[i].innerHTML='<div style="position:absolute;z-index:5;display:none;">'+g+'</div><span class="podpis" id="po'+g+'" style="font-size:1em;font-family:Courier New;position:relative;margin: auto;z-index:-1;margin-top:0px;opacity: 0.3;text-align: center; font-family:Courier New;font-weight: bold;font-size:1.5em;top:calc(50% - 30px);"></span>';
				}else{
					canva[i].innerHTML='<div style="position:absolute;z-index:5;display:none;">'+g+'</div><span class="podpis" id="po'+g+'" style="font-size:1em;font-family:Courier New;position:relative;margin: auto;z-index:-1;margin-top:0px;opacity: 0.3;text-align: center; font-family:Courier New;font-weight: bold;font-size:1.5em;top:calc(50% - 30px);"></span>';			
				}
			}
	...
	document.getElementById('text_help').innerHTML="С начала введите размеры нужных вам блоков, разместите фото, а затем нажмите \"обрезать лишнее <img width=\"24px\" height=\"24px\" src=\"img/split.png\">.\"";
}
...
function add_input(){
	...
	if(count>10000-parseInt(wid1)){document.getElementById('width').style.display='block';
	setTimeout("document.getElementById('width').style.display='none'", 3000);}
	//document.getElementById('width').setAttribute('display','block');
	//alert('общая ширина 7000мм')}
	else{
	...
	if(parseInt(wid1)<1000 || parseInt(hei1)<400){document.getElementById('po'+co).innerHTML=''}else{document.getElementById('po'+co).innerHTML='';}
	}
...
sm_div.innerHTML='<span style="position:absolute;z-index:5;display:none;">'+kol+'</span>';
...
var heig=300;//document.getElementById('height'+kol).value;
//heig=heig/6;
var bl='отступ';
wid=600  //wid*6;
...
tr.innerHTML='<td id="ko'+kol+'" class="che"><img src="img/but/triangle.png">'+kol+'. </td><td id="ko2'+kol+'" style="border-right:1px dotted;"><input value="'+heig+'" class="width" name="height'+kol+'" id="height'+kol+'" type="text" step="50" onkeyup="resize_y(\'height'+kol+'\', '+kol+')"></td><td id="ko3'+kol+'"><input class="width" id="width'+kol+'" name="width_'+kol+'" type="text" onkeyup="resize_x(\'width'+kol+'\', '+kol+')"  value="'+wid+'"></td><td style="color:#009746" id="ko4'+kol+'"><input type="text" placeholder="'+bl+'" class="width" id="block'+kol+'" name="block'+kol+'" onkeyup="ize('+kol+', '+id+')"></td><td id="ko'+kol+'"><a class="crop" onclick="crop_div('+kol+')"><img height="16px" src="img/but/del.png"></a></td>';
tr.id=kol;
...
function resize_x(name, id){
	...
	var vae=/\D+/;
	var wws=vae.test(val);
	if(wws){document.getElementById('width'+id).value=parseInt(val.replace(/\D+/g,""));}
	...
	if(su>10001){ 
		...
		var dsa =document.getElementById('canvas'+id).offsetWidth;
		dsa=parseInt(dsa)*6;
		document.getElementById('width'+id).value=dsa.toFixed();
...
function change_sm(id, top, left, width, height){
 ...
 function resize_y(name, id){
	...
	var val=document.getElementById('height'+id).value;
		var vae=/\D+/;
	var wws=vae.test(val);
	if(wws){val=parseInt(val.replace(/\D+/g,""));document.getElementById('height'+id).value=val;}
...
//				alert(idc);
				...
				if(nam[0]=='.'){
					var val=document.getElementById('height'+id).value;
					var to = document.getElementById('canvas'+nam[1]).offsetTop;
					var ttt = parseInt(to)-parseInt(height);
					if(val>800){
						ttt=parseInt(to)-100;
						document.getElementById('height'+id).value='600';
						document.getElementById('canvas'+id).style.height='100';
						document.getElementById('canvas'+id).style.top=parseInt(to)-100;
						var to = document.getElementById('canvas'+id).style.top;
						var left = document.getElementById('canvas'+id).style.left;
						var width = document.getElementById('canvas'+id).style.width;
						var height = document.getElementById('canvas'+id).style.height;
